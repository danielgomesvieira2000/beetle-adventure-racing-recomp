...
#                   endif
        ...
        // BAR: the end of a full synchronization is the frame boundary for the port's HUD inspector
        // -- the elements noted since the last one are complete, and may be swapped in for the panel
        // to read. No-op when the port has installed nothing.
        //
        // Deliberately here rather than inside advanceWorkload: the other caller is the paused path,
        // which re-submits the last workload every frame without any display list having been
        // processed, so ending a frame there would publish an empty list and blank the panel for as
        // long as the game stayed paused -- exactly when it is being read.
        BarHud::endFrame();
    ...
    void State::inspect() {
        ...
        */
        ...
        // BAR: the port's own inspector, drawn into this frame's UI beside RT64's. RT64 owns the
        // ImGui context, and both halves have to be in one window or the tool is no better than
        // reading two logs side by side -- RT64 answers "what did the renderer draw here", and the
        // port answers "which of the game's elements is that, and what class did we give it".
        // Null unless the port installs it; see src/hle/rt64_bar_hud.h.
