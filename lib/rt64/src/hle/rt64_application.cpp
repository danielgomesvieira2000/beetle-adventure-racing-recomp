    ...
    bool Application::windowMessageFilter(unsigned int message, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_KEYDOWN: {
            ...
            case VK_F1:
                ...
                return true;
            // BAR: F2 is deliberately unbound. The port turns developer mode on in every build so
            // that F1 opens the debug menu, which arms these shortcuts for everyone; a session-wide
            // ray tracing toggle with nothing on screen to explain what changed is not something to
            // leave under a finger. F1 (the menu), F3 (view RDRAM) and F4 (texture replacements) are
            // either visibly reversible or plainly diagnostic, so they stay. With no case for it the
            // key is not filtered, and passes through to the game like any other.
            case VK_F3:
