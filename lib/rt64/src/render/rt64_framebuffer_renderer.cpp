...
#   endif
        ...
        // BAR_DBG_ASPECT=1 reports whether the widescreen adjustment is actually being applied to
        // this framebuffer, and with what scale. "Expand does nothing on screen X" can mean the
        // scissor failed the similarity test, or the target ratio collapsed to the source, or the
        // scale is applied but the game's own projection ignores it -- these look identical on
        // screen and completely different here. Logged only when the numbers change.
        {
            static const bool aspDbg = std::getenv("BAR_DBG_ASPECT") != nullptr;
            if (aspDbg) {
                static float lastRatio = -1.0f, lastScale = -1.0f, lastSrc = -1.0f, lastTgt = -1.0f;
                static bool lastAdjust = false;
                if ((scissorRatio != lastRatio) || (aspectRatioScale != lastScale) || (adjustRatio != lastAdjust) ||
                    (p.aspectRatioSource != lastSrc) || (p.aspectRatioTarget != lastTgt)) {
                    lastRatio = scissorRatio; lastScale = aspectRatioScale; lastAdjust = adjustRatio;
                    lastSrc = p.aspectRatioSource; lastTgt = p.aspectRatioTarget;
                    fprintf(stderr, "[aspect] scissor=%.0fx%.0f ratio=%.4f source=%.4f target=%.4f adjust=%d scale=%.4f extPct=%.2f\n",
                        float(fbPair.scissorRect.width(false, true)), float(fbPair.scissorRect.height(false, true)),
                        scissorRatio, p.aspectRatioSource, p.aspectRatioTarget, adjustRatio ? 1 : 0,
                        aspectRatioScale, p.extAspectPercentage);
                    fflush(stderr);
                }
            }
        }
...
#       endif
            ...
            // BAR: see the matching call in ProjectionProcessor. The two must ask for the same
            // question of the same projection and get the same answer, or the orthographic layer is
...
#               endif
                    {
                        ...
                        // BAR: an anchored layer's scissor has to reach the frame's edges with it. The
                        // game scissors its HUD to the whole 320x240 screen, which converts to the
                        // centred 4:3 region, so a layer moved to an edge is drawn outside its own
                        // scissor and clipped away -- which is exactly how the speedometer's needle
                        // disappeared while its dial anchored correctly.
                        const uint16_t scissorLeftOrigin = barAnchoredViewport ? uint16_t(G_EX_ORIGIN_LEFT) : call.callDesc.scissorLeftOrigin;
                        const uint16_t scissorRightOrigin = barAnchoredViewport ? uint16_t(G_EX_ORIGIN_RIGHT) : call.callDesc.scissorRightOrigin;
