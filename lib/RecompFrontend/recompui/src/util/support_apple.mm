...
namespace recompui::file {
    void apple::dispatch_on_ui_thread(std::function<void()> func) {
        dispatch_async(dispatch_get_main_queue(), ^{
            func();
        });
    }

    std::optional<std::filesystem::path> apple::get_application_support_directory() {
    ...
    std::filesystem::path apple::get_bundle_resource_directory() {
        NSString *bundlePath = [[NSBundle mainBundle] resourcePath];
        return std::filesystem::path([bundlePath UTF8String]);
    }

    std::filesystem::path apple::get_bundle_directory() {
